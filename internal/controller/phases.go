...
func (p *PhaseReconciler) PreflightChecks(ctx context.Context) (*Result, error) {
	return &Result{}, preflightChecks(ctx, p.ctrlClient, p.provider, p.providerList, p.providerTypeMapper)
}

// ProviderPreflightChecks a wrapper around the regular provider preflight checks.
func (p *PhaseReconciler) ProviderPreflightChecks(ctx context.Context) (*Result, error) {
	return &Result{}, providerPreflightChecks(ctx, p.ctrlClient, p.provider, p.providerList)
}

// WaitForCoreReady a wrapper around the preflight checks.
func (p *PhaseReconciler) WaitForCoreReady(ctx context.Context) (*Result, error) {
	return &Result{}, waitForCoreReady(ctx, p.ctrlClient, p.provider)
}

// CheckGithubToken a wrapper around the github token check.
func (p *PhaseReconciler) CheckGithubToken(ctx context.Context) (*Result, error) {
	return &Result{}, checkGithubToken(ctx, p.ctrlClient, p.provider)
}

// CheckPredefinedProvider a wrapper around the provider fetchConfig check.
func (p *PhaseReconciler) CheckPredefinedProvider(ctx context.Context) (*Result, error) {
	return &Result{}, checkPredefinedProvider(ctx, p.provider, p.providerTypeMapper)
}

// CorePreflightChecks a wrapper around the core provider checks.
func (p *PhaseReconciler) CorePreflightChecks(ctx context.Context) (*Result, error) {
	return &Result{}, corePreflightChecks(ctx, p.ctrlClient, p.provider, p.providerList)
}
