...
func TestPreflightChecks(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := preflightChecks(context.Background(), fakeClient, tc.providers[0], tc.providerList, util.ClusterctlProviderType)
			if tc.expectedError {
...
func TestPreflightChecksUpgradesDowngrades(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := preflightChecks(context.Background(), fakeClient, provider, &operatorv1.CoreProviderList{}, util.ClusterctlProviderType)
			if tc.expectedError {
				gs.Expect(err).To(HaveOccurred())
			} else {
				gs.Expect(err).ToNot(HaveOccurred())
